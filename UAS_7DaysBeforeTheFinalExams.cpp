...
vector<Question> loadQuestions() {
    ...
    while (getline(file, line)) {
        // Hapus karakter carriage return '\r' jika ada (biasanya masalah Windows/Linux)
        if (!line.empty() && line.back() == '\r') {
            line.pop_back();
        }

        // Jika baris kosong, berarti satu blok soal selesai
        if (line.empty()) {
            if (buffer.size() >= 6) { // Minimal 1 baris soal + 4 opsi + 1 kunci = 6 baris
                Question q;
                
                // 1. Ambil Kunci Jawaban (Baris Terakhir di buffer)
                string keyStr = buffer.back();
                if (!keyStr.empty()) q.answerKey = keyStr[0];
                buffer.pop_back();

                // 2. Ambil Opsi D, C, B, A (Urutan mundur dari bawah)
                q.d = buffer.back(); buffer.pop_back();
                q.c = buffer.back(); buffer.pop_back();
                q.b = buffer.back(); buffer.pop_back();
                q.a = buffer.back(); buffer.pop_back();

                // 3. Sisanya di buffer adalah Teks Soal (bisa multi-line)
                for (size_t i = 0; i < buffer.size(); i++) {
                    q.text += buffer[i];
                    if (i < buffer.size() - 1) q.text += "\n"; // Tambah enter antar baris soal
                }
                
                questions.push_back(q);
            }
            buffer.clear(); // Bersihkan buffer untuk soal berikutnya
        } else {
            buffer.push_back(line); // Simpan baris ke buffer sementara
        }
    }

    // CEK BLOK TERAKHIR (Jika file tidak diakhiri enter/baris kosong)
    if (!buffer.empty() && buffer.size() >= 6) {
        Question q;
        string keyStr = buffer.back();
        if (!keyStr.empty()) q.answerKey = keyStr[0];
        buffer.pop_back();
...
bool loadGame(PlayerStats* p) {
    if (!p) return false; // null pointer check
